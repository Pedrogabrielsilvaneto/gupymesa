...
window.GupyBiblioteca = {
    ...
    init: async function () {
        console.log("📚 Biblioteca: Inicializando Versão V.1.1.0 (TiDB)");
        if (window.Sistema) {
    ...
    carregarFrases: async function () {
        try {
            ...
            if (this.usuario) {
                ...
                if (stats) stats.forEach(s => meusUsosMap[s.frase_id] = s.qtd_uso);
            }
            ...
            <div id="card-frase-${f.id}" class="flex flex-col bg-white rounded-2xl shadow-sm border border-slate-100 border-l-4 ${colors.card} hover:shadow-md transition-all duration-300 group overflow-hidden">
                <div class="px-5 pt-4 pb-2 flex justify-between items-start">
                    <div class="flex flex-col gap-1.5">
                        <span onclick="GupyBiblioteca.filtrarPorEmpresa('${f.empresa || 'Geral'}')" class="${empColor} text-[10px] font-black px-2 py-0.5 rounded-md border uppercase tracking-wider cursor-pointer shadow-sm">${f.empresa || 'Geral'}</span>
    ...
    copiarTexto: async function (id) {
        ...
        navigator.clipboard.writeText(f.conteudo).then(async () => {
            ...
            await this.callAPI({
                ...
                data: { usos: novoUso, ultimo_uso: new Date().toISOString() }
            });
            f.usos = novoUso;
    ...
    registrarLog: async function (acao, desc) {
        ...
        await this.callAPI({
            ...
            data: { usuario: String(this.usuario.id), acao: acao, detalhe: desc, data_hora: new Date().toISOString() }
        });
    },
    ...
    // Métodos Auxiliares de UI e Utilitários
    normalizar: t => (t || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""),
    
    atualizarRodape: function () {
        const ver = (window.CONFIG && CONFIG.VERSION) ? CONFIG.VERSION : 'V.1.1.0';
    ...
    aplicarFiltros: function (scrollToTop, originIsSearch) {
        ...
        if (termo || valEmpresa || valMotivo || valDoc) {
            ...
            filtrados = filtrados.slice(0, 20); // Ranking padrão
        }
    ...
    getEmpresaColor: function (emp) {
        ...
        if (e.includes('CLARO')) return 'bg-red-500 text-white';
        if (e.includes('TIM')) return 'bg-blue-600 text-white';
        if (e.includes('VIVO')) return 'bg-indigo-600 text-white';
    ...
    atualizarSugestoesModal: function () {} // Placeholder
};
