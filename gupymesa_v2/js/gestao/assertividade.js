...
Gestao.Assertividade = {
    ...
    renderTabela: function (dados) {
        ...
        const html = dados.map(d => {
            ...
            const docNameFormatado = Gestao.Assertividade.fixText(d.doc_name || '-');
            const obsFormatada = Gestao.Assertividade.fixText(d.observacao || '');
            return `
                <tr class="hover:bg-slate-50 border-b border-slate-100 text-[11px] text-slate-600">
                    ...
                    <td class="px-2 py-2 text-slate-500 truncate max-w-[150px]">${docNameFormatado}</td>
                    <td class="px-2 py-2 text-center"><span class="px-1.5 py-0.5 rounded text-[9px] font-bold border ${classeStatus}">${st || 'ND'}</span></td>
                    <td class="px-2 py-2 text-slate-400 max-w-[100px]" title="Clique para expandir/recolher observação">
                        <div class="truncate cursor-pointer hover:text-blue-500 transition-colors" onclick="this.classList.toggle('truncate'); this.classList.toggle('whitespace-normal'); event.stopPropagation();">
                            ${obsFormatada}
                        </div>
                    </td>
