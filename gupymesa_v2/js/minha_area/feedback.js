MinhaArea.Feedback = {
    ...
    renderizarContatos: function (lista) {
        ...
        const html = lista.map(u => {
            ...
            const avatar = u.avatar_url
                ...
                : `<span class="text-xs font-black text-white">${iniciais}</span>`;

            const cargo = u.funcao || u.perfil || 'Colaborador';
            const cores = ['bg-violet-600','bg-rose-600','bg-amber-600','bg-teal-600','bg-indigo-600','bg-pink-600'];
            const cor = cores[u.nome.charCodeAt(0) % cores.length];

            return `
                <div onclick="MinhaArea.Feedback.selecionarChat('${u.id}')" id="contact-item-${u.id}"
                    class="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-white/8 transition border-b border-white/5">
                    <div class="w-11 h-11 rounded-full ${cor} flex items-center justify-center shrink-0 overflow-hidden shadow">
                        ${avatar}
                    </div>
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-center">
                            <span class="font-bold text-white text-sm truncate capitalize">${u.nome.toLowerCase()}</span>
                        </div>
                        <p class="text-[11px] text-slate-400 truncate mt-0.5">${cargo}</p>
                    </div>
                </div>
            `;
        }).join('');
    ...
    selecionarChat: async function (id) {
        ...
        document.querySelectorAll('[id^="contact-item-"]').forEach(el => {
            el.classList.remove('bg-white/15', 'border-l-2', 'border-blue-400');
        });
        const activeItem = document.getElementById(`contact-item-${id}`);
        if (activeItem) activeItem.classList.add('bg-white/15', 'border-l-2', 'border-blue-400');
    ...
    carregarMensagens: async function () {
        try {
            ...
            if (this.chatAtivo === 'GERAL') {
                sql = `
                    SELECT f.*, u.nome as remetente_nome, u.avatar_url as remetente_avatar
                    FROM feedbacks f
                    LEFT JOIN usuarios u ON f.remetente_id = u.id
                    WHERE f.tipo_destinatario = 'TODOS' 
                ...
                sql = `
                    ...
                    FROM feedbacks f
                    LEFT JOIN usuarios u ON f.remetente_id = u.id
                    WHERE (f.remetente_id = ? AND f.destinatario_id = ?) 
                       OR (f.remetente_id = ? AND f.destinatario_id = ?) 
                    ORDER BY f.created_at ASC
                `;
    ...
    renderizarMensagens: function () {
        ...
        this.mensagensCache.forEach(msg => {
            const dataMsg = new Date(msg.created_at);
            const dataString = dataMsg.toLocaleDateString();

            // Adiciona Separador de Data (WhatsApp Style)
            if (dataString !== ultimaData) {
                let labelData = '';
                const hoje = new Date();
                const ontem = new Date();
                ontem.setDate(hoje.getDate() - 1);

                if (dataString === hoje.toLocaleDateString()) {
                    labelData = 'Hoje';
                } else if (dataString === ontem.toLocaleDateString()) {
                    labelData = 'Ontem';
                } else {
                    labelData = dataMsg.toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' });
                }

                html += `
                    <div class="flex justify-center my-4">
                        <span class="bg-white/80 backdrop-blur-sm px-3 py-1 rounded-lg text-[10px] font-bold text-slate-500 shadow-sm uppercase tracking-wider">
                            ${labelData}
                        </span>
                    </div>
                `;
                ultimaData = dataString;
            }
            ...
            const bubbleClass = souEu
                ? 'bg-[#dcf8c6] text-slate-800 rounded-br-sm shadow-sm'
                : 'bg-white text-slate-800 rounded-bl-sm shadow-sm';
            
            const time = dataMsg.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
            ...
            if (msg.tipo_midia === 'AUDIO') {
                conteudo = `
                    <div class="flex items-center gap-2 py-1">
                        ...
                        <audio src="${msg.url_midia}" controls class="h-8 max-w-[180px]"></audio>
                    </div>`;
            } else if (msg.tipo_midia === 'IMAGEM') {
                conteudo = `<div class="text-xs text-slate-400 italic">[Imagem não suportada]</div>`;
            }

            if (msg.mensagem) {
                conteudo += `<p class="whitespace-pre-wrap leading-relaxed text-[13px]">${Sistema.escapar(msg.mensagem)}</p>`;
            }

            let nomeRemetente = '';
            if (this.chatAtivo === 'GERAL' && !souEu) {
                const coresNome = ['text-violet-500', 'text-rose-500', 'text-amber-500', 'text-teal-500', 'text-indigo-500'];
                const corNome = coresNome[(msg.remetente_nome || '').charCodeAt(0) % coresNome.length];
                nomeRemetente = `<div class="text-[11px] font-bold ${corNome} mb-0.5 truncate">${msg.remetente_nome || 'Usuário'}</div>`;
            }

            html += `
                <div class="flex flex-col max-w-[85%] sm:max-w-[70%] ${alignClass} animate-fade-in mb-1">
                    <div class="${bubbleClass} px-3 py-1.5 rounded-2xl relative min-w-[80px]">
                        ${nomeRemetente}
                        ${conteudo}
                        <div class="flex items-center justify-end gap-1 mt-0.5">
                            <span class="text-[9px] text-slate-400 whitespace-nowrap">${time}</span>
                            ${souEu ? '<i class="fas fa-check-double text-[9px] text-blue-400"></i>' : ''}
                        </div>
                    </div>
                </div>
            `;
