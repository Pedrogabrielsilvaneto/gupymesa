...
Produtividade.Performance = {
    ...
    // Helper para normalizar datas vindas do TiDB (ex: '2026-02-03T00:00:00.000Z' -> '2026-02-03')
    normData: function (d) {
        if (!d) return '';
        return String(d).includes('T') ? String(d).split('T')[0] : String(d).split(' ')[0];
    },
    ...
    carregar: async function () {
        ...
        try {
            // 1. KPIs Gerais (Substitui o RPC quebrado por queries diretas via Sistema.query)
            const sqlKPIs = `
                SELECT 
                    u.id AS usuario_id,
                    u.nome,
                    ...
                    COALESCE(a.soma_auditorias, 0) AS soma_auditorias,
                    COALESCE(a.qtd_auditorias, 0) AS qtd_auditorias
                FROM usuarios u
                LEFT JOIN (
                    SELECT usuario_id, SUM(quantidade) AS total_qty
                    FROM producao
                    WHERE data_referencia >= ? AND data_referencia <= ?
                    GROUP BY usuario_id
                ) p ON u.id = p.usuario_id
                LEFT JOIN (
                    SELECT CAST(usuario_id AS CHAR) AS uid, 
                           SUM(assertividade_val) AS soma_auditorias, 
                           COUNT(*) AS qtd_auditorias
                    FROM assertividade
                    WHERE data_referencia >= ? AND data_referencia <= ?
                      AND assertividade_val IS NOT NULL
                    GROUP BY usuario_id
                ) a ON CAST(u.id AS CHAR) = a.uid
                WHERE u.ativo = true OR u.ativo = 1
            `;
            ...
            const [dataKPIs, dataTimeline, dataDocs] = await Promise.all([
                Sistema.query(sqlKPIs, [datas.inicio, datas.fim, datas.inicio, datas.fim]),
                Sistema.query(sqlTimeline, [datas.inicio, datas.fim]),
                Sistema.query(sqlDocs, [datas.inicio, datas.fim])
            ]);

            this.dadosRPC = dataKPIs || [];
            this.dadosTimeline = dataTimeline || [];
            this.dadosDocs = dataDocs || [];
    ...
    processarDadosAtuais: function () {
        ...
        // [FIX] Documentos devem respeitar os filtros contextuais (Contrato/Função/Nome)
        const activeAssistants = new Set(rpcFiltrado.map(u => (u.nome || '').toLowerCase()));

        const docsFiltrados = this.usuarioSelecionado
            ? this.dadosDocs.filter(d => d.assistente_nome && d.assistente_nome.toLowerCase().includes(nomeUsuarioSel.split(' ')[0].toLowerCase()))
            : this.dadosDocs.filter(d => {
                const docAssistant = (d.assistente_nome || '').toLowerCase();
                return docAssistant && activeAssistants.has(docAssistant);
            });
        ...
        producaoFiltrada.forEach(p => {
            const dataKey = this.normData(p.data_referencia);
            prodPorDia[dataKey] = (prodPorDia[dataKey] || 0) + (Number(p.quantidade) || 0);
        });
        ...
        docsFiltrados.forEach(d => {
            ...
            if (isNok) {
                ...
                const dataKey = this.normData(d.data_referencia);
                noksPorDia[dataKey] = (noksPorDia[dataKey] || 0) + 1;
            }
        });
    ...
    renderizarCenario: function () {
        ...
        // 1. Filtragem Inicial para o Sidebar
        let rpcFiltrado = (window.Produtividade.Filtros && typeof window.Produtividade.Filtros.preFiltrar === 'function')
            ? window.Produtividade.Filtros.preFiltrar(this.dadosRPC)
            : this.dadosRPC;
        ...
        <div class="grid grid-cols-12 gap-6 h-full animate-fade-in">
            <div class="col-span-12 lg:col-span-3 flex flex-col gap-4">
                ${this.renderSidebar(stats, rpcFiltrado)}
            </div>
    ...
    renderSidebar: function (stats, rpcFiltrado) {
        const isGlobal = !this.usuarioSelecionado;

        // [FIX] Sidebar deve usar os dados FILTRADOS pelo HUD (CLT/PJ/Função)
        const sorted = [...rpcFiltrado]
            .filter(u => {
                const cargo = (u.funcao || '').toUpperCase();
                const perfil = (u.perfil || '').toUpperCase();
                return !['GESTOR', 'GESTORA', 'AUDITOR', 'AUDITORA', 'ADMIN', 'COORDENADOR'].some(t => cargo.includes(t) || perfil.includes(t));
            })
            .map(u => {
                const qty = Number(u.total_qty) || 0;
    ...
    renderChartEvolution: function (stats) {
        ...
        if (this.mode === 'tracao') {
            ...
            raw.forEach(r => { const dk = this.normData(r.data_referencia); dataMap[dk] = (dataMap[dk] || 0) + Number(r.quantidade); });
        } else {
            ...
            rawDocs.forEach(d => {
                ...
                if (status.includes('NOK') || status.includes('REPROV')) {
                    const dk = this.normData(d.data_referencia);
                    dataMap[dk] = (dataMap[dk] || 0) + 1;
                }
            });
        }
