/* ARQUIVO: js/minha_area/relatorios.js
   DESCRIÇÃO: Módulo de Relatórios da Minha Área - V6.4.1 (Gráfico Comparativo)
   LAST_DEPLOY: 2026-05-04 21:48
*/
...
MinhaArea.Relatorios = {
    ...
    _selectedGapUsers: new Set(),
    _gapBenchmarkIds: new Set(),
    _currentGapChartType: 'line', // 'line' ou 'bar'
    ...
    setGapChartType: function(type) {
        this._currentGapChartType = type;
        this.abrirGraficoComparativo();
    },
    ...
    abrirGraficoComparativo: function() {
        ...
        setTimeout(() => {
            ...
            const chartType = this._currentGapChartType || 'line';
            ...
            // Configuração para Gráfico de BARRAS DEITADAS (Horizontal)
            if (chartType === 'bar' && !isSingle) {
                // Calcula a média do período para cada usuário
                const barData = usersToDraw.map(u => {
                    const vals = meses.map(m => u.meses[m] || 0).filter(v => v > 0);
                    const avg = vals.length > 0 ? vals.reduce((a, b) => a + b, 0) / vals.length : 0;
                    return { nome: u.nome, avg, id: u.id };
                }).sort((a, b) => b.avg - a.avg);

                this._gapChartInstance = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: barData.map(d => d.nome),
                        datasets: [{
                            label: 'Média no Período',
                            data: barData.map(d => Math.round(d.avg)),
                            backgroundColor: barData.map(d => (d.id === 'virtual_bench' || this._gapBenchmarkIds.has(d.id)) ? benchColor : '#3b82f6'),
                            borderRadius: 6,
                            barThickness: 24
                        }]
                    },
                    options: {
                        ...
                        plugins: {
                            ...
                            tooltip: { 
                                callbacks: { 
                                    label: (ctx) => `${ctx.raw} metas/dia (Média)` 
                                } 
                            }
                        },
                        scales: {
                            x: { beginAtZero: true, grid: { color: '#f1f5f9' }, title: { display: true, text: 'Metas/Dia (Média)', font: { weight: 'bold' } } },
            ...
            <div class="bg-white w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div class="bg-slate-50 border-b border-slate-100 px-6 py-4 flex justify-between items-center shrink-0">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h3 class="font-black text-slate-800 text-xs uppercase tracking-widest leading-tight">Visualização de GAP</h3>
                            <p id="chart-user-name" class="text-[10px] text-slate-400 font-bold uppercase">Comparativo Geral de Performance</p>
                        </div>
                    </div>

                    <!-- Toggle de Tipo de Gráfico -->
                    <div class="flex bg-slate-200/50 p-1 rounded-xl border border-slate-200">
                        <button onclick="MinhaArea.Relatorios.setGapChartType('line')" 
                                class="px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${this._currentGapChartType === 'line' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-600'}">
                            <i class="fas fa-chart-line mr-1"></i> Tendência
                        </button>
                        <button onclick="MinhaArea.Relatorios.setGapChartType('bar')" 
                                class="px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${this._currentGapChartType === 'bar' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400 hover:text-slate-600'}">
                ...
                <div class="p-8 overflow-y-auto custom-scrollbar flex-1">
                    ...
                    <div id="gap-summary-container" class="mt-6 pt-6 border-t border-slate-100 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    </div>
                </div>
            </div>
        `;
