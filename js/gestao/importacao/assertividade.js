...
Gestao.Importacao.Datas = {
    ...
    calcularCompetencia: function (dataISO) {
        ...
        // Regra do 1º dia do mês (domingo): deve ser considerado no mês em que ele próprio pertence
        // Ou seja, não volta pra trás (mês anterior), vai pro próximo dia útil
        if (data.getDate() === 1 && data.getDay() === 0) {
        ...
        // Todos os dados que caem no fim de semana devem ser considerados na sexta-feira anterior
        // Inclusive se cair num feriado e for pra trás.
...
Gestao.Importacao.Assertividade = {
    ...
    /**
     * Detecta se o arquivo é UTF-8 ou ISO-8859-1 (Latin-1/Windows-1252)
     */
    detectarEncoding: function (file) {
        return new Promise((resolve) => {
            ...
            reader.onload = function (e) {
                ...
                // 1. Check for UTF-8 BOM (Byte Order Mark)
                if (bytes[0] === 0xEF && bytes[1] === 0xBB && bytes[2] === 0xBF) {
                    resolve("UTF-8");
                    return;
                }
                // 2. Smart check for UTF-8 valid sequences (fatal=true will throw on invalid bytes)
                try {
                    const decoder = new TextDecoder('utf-8', { fatal: true });
                    decoder.decode(bytes);
                    resolve("UTF-8");
                } catch (err) {
                    // If it fails, it's likely ISO-8859-1 (or Windows-1252)
                    resolve("ISO-8859-1");
                }
            };
            // Read first 10KB to check encoding (usually enough to catch any accent)
    ...
    processarArquivo: async function (input) {
        ...
        this.injetarModal();
        await this.mostrarProgresso(0, "Detectando codificação...", 'normal');
        
        const encoding = await this.detectarEncoding(arquivo);
        console.log(`📡 GupyMesa: Codificação detectada para ${arquivo.name} -> ${encoding}`);

        await this.mostrarProgresso(5, `Codificação: ${encoding}<br>Lendo CSV (Modo Rápido)...`, 'normal');
        ...
        for (let i = 0; i < linhasBrutas.length; i += chunkSize) {
            ...
            chunk.forEach((row) => {
                try {
                    ...
                    const empresaNome = this.limparTexto(getVal('Empresa'));
                    const assistenteNome = this.limparTexto(getVal('Assistente'));
                    const idPpc = this.limparBigIntString(getVal('ID PPC'));
                    let userId = this.limparInteiro(getVal('id_assistente'));

                    // 🔥 FILTRO DE SEGURANÇA GUPYMESA:
                    ...
                    const registro = {
                        id_ppc: idPpc, data_referencia: dataCompetencia, end_time_raw: rawTime, usuario_id: userId,
                        company_id: this.limparBigIntString(getVal('Company_id')), schema_id: this.limparBigIntString(getVal('Schema_id')),
                        empresa_nome: empresaNome, assistente_nome: assistenteNome,
...
console.log("🚀 Importador V122 (Security Guard Restored) carregado.");